 ...
 *				                  Copyright (C) 2019 by Synapsys L.L.C.					            *
 *							            All rights reserved								            *
 *--------------------------------------------------------------------------------------------------*
 * WARNING: This firmware is protected by copyright law and international treaties. Unauthorized    *
 * reproduction or distribution of this firmare or any portion of it is prohibited. Severe civil    *
 ...
 * Parameters:                                                                                          *
 *      spiNbr: 1 = SPI1, 2 = SPI2, 3 = SPI3 4 = SPI4, 5 = SPI5, 6 = SPI6                               *
 *      speed: SPI_SPEED_SLOW or SPI_SPEED_FAST                                                         *
 * Note: This routine initializes the SPI port I/O. It DOES NOT initialize the port for any particular  *
 *       device (such as an SD interface).                                                              *
 * Calling this routine results in the SPI port being setup as follows:                                 *
 *      SPI clock source is PERCLK (64mHz).                                                             *
 *          if SLOW speed use div by 256 resulting in a clk of 250000 Hz.                               *
 *          else use div by 2 resulting in a clk of 32mHz.                                              *
 *      SPI setup: Master mode, Module enabled, Clock data on falling edge, clk=1 on idle.              *
...
void spiSDInit(void)
{
    ...
    if (!spi1IsInitialized)
    {
        ...
        uint32_t clkEnbl = RCC_APB2ENR;
        if (clkEnbl) { }
        
        sreg = SPI2S_SR(SPI1);
        if (sreg) { }
        cfg2reg = SPI_CFG2(SPI1);
        if (cfg2reg) { }
